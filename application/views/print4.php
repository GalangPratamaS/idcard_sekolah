...
<body style="font-family: arial;font-size: 12px;position:absolute;">
	<div id="idcard" style="width: 1044px;height: 673px;margin: 50px;background-image: url('<?= base_url('asset/kartu/desain/alazhar_depan.png'); ?>');">
		...
		<img style="border-radius:5px; solid #ffffff;position: absolute;margin-left: 68px;margin-top: -13px;" alt="foto" src="<?= base_url('asset/kartu/foto/') . $s['foto_siswa']; ?>" width="155px" height="200px">
		<table style="margin-top: 27px;padding-left: 250px; position: relative;font-family: arial;font-size: 26px;color:#ffffff;">
			<tr>
				...
				if(strlen($s['nama_siswa']) > 21){
				echo $this->CI->cutText(ucwords(strtolower($s['nama_siswa'])),22);
				} else {
					echo ucwords(strtolower($s['nama_siswa']));
				}  ?></td>
			</tr>
			<tr>
				<td style="padding-right: 20px;text-align: left;">NIS/NISN</td>
				<td>:</td>
				<td style="text-align: left;"><?= $s['nomor_induk_siswa']; ?></td>
			</tr>
			</tr>			
			<tr>
				...
				<td>:</td>
				<td style="text-align: left;"><?php 
				if(strlen($s['alamat']) > 23){
					echo ucwords(strtolower($this->CI->cutText($s['alamat'],24)));
				} else {
					echo ucwords(strtolower($s['alamat']));
				}
				 ?></td>
			</tr>			
		</table>
		<?php
		$kode_kartu = $s['nomor_kartu'];
		...
		echo '<img class="barcode img-responsive img" src="data:image/png;base64,' . base64_encode($generator->getBarcode($s['nomor_kartu'], $generator::TYPE_CODE_128)) . '">'; ?>
