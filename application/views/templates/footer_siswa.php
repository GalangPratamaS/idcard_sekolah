<footer class="footer">
    2020 Al Azhar Karawang
    <span class="d-md-inline-block"> ©
        <a href="https://alazhar-karawang.sch.id" class="text-success">alazhar-karawang.sch.id</a></span>
</footer>
</div>
...
<script>
    ...
    $(document).ready(function() {
        $("#form_update").on("submit", function(event) {
            ...
            dataType: 'json',
                ...
                if (json.status == 'success') {
                   ...
                   $('#uploaded_file').html('<div class="alert alert-danger">Data gagal diupload, pastikan file dan form sudah di isi!</div>');
        ...
        $("#form_stnk").on("submit", function(event) {
            event.preventDefault();
            $.ajax({
               type: 'POST',
            url: '<?= base_url('Siswa/updateSTNK') ?>',
            data: new FormData(this),
            contentType: false,
            cache: false,
            async:false,
